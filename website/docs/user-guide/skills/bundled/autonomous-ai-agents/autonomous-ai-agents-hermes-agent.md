---
title: "OpenClaw — Configure, extend, or contribute to OpenClaw"
sidebar_label: "OpenClaw"
description: "Configure, extend, or contribute to OpenClaw"
---
...
Configure, extend, or contribute to OpenClaw.
...
OpenClaw is an open-source AI agent framework by Nous Research that runs in your terminal, messaging platforms, and IDEs. It belongs to the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. OpenClaw works with any LLM provider (OpenRouter, Anthropic, OpenAI, DeepSeek, local models, and 15+ others) and runs on Linux, macOS, and WSL.

What makes OpenClaw different:

- **Self-improving through skills** — OpenClaw learns from experience by saving reusable procedures as skills. When it solves a complex problem, discovers a workflow, or gets corrected, it can persist that knowledge as a skill document that loads into future sessions. Skills accumulate over time, making the agent better at your specific tasks and environment.
- **Persistent memory across sessions** — remembers who you are, your preferences, environment details, and lessons learned. Pluggable memory backends (built-in, Honcho, Mem0, and more) let you choose how memory works.
- **Multi-platform gateway** — the same agent runs on Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Email, and 10+ other platforms with full tool access, not just chat.
- **Provider-agnostic** — swap models and providers mid-workflow without changing anything else. Credential pools rotate across multiple API keys automatically.
- **Profiles** — run multiple independent OpenClaw instances with isolated configs, sessions, skills, and memory.
- **Extensible** — plugins, MCP servers, custom tools, webhook triggers, cron scheduling, and the full Python ecosystem.

People use OpenClaw for software development, research, system administration, data analysis, content creation, home automation, and anything else that benefits from an AI agent with persistent context and full system access.

**This skill helps you work with OpenClaw effectively** — setting it up, configuring features, spawning additional agent instances, troubleshooting issues, finding the right commands and settings, and understanding how the system works when you need to extend or contribute to it.

**Docs:** https://openclaw.nousresearch.com/docs/
...
Platform docs: https://openclaw.nousresearch.com/docs/user-guide/messaging/
...
authoritative list or see the [live slash commands reference](https://openclaw.nousresearch.com/docs/reference/slash-commands).
...
Full config reference: https://openclaw.nousresearch.com/docs/user-guide/configuration
...
Full provider docs: https://openclaw.nousresearch.com/docs/integrations/providers
...
Common "why is OpenClaw doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `hermes` invocation) because they're read once at startup.
...
By default (`approvals.mode: manual`), OpenClaw prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
## Spawning Additional OpenClaw Instances

Run additional OpenClaw processes as fully independent subprocesses — separate sessions, tools, and environments.
...
OpenClaw uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
User docs: https://openclaw.nousresearch.com/docs/user-guide/features/cron
...
User docs: https://openclaw.nousresearch.com/docs/user-guide/features/curator
...
User docs: https://openclaw.nousresearch.com/docs/user-guide/features/kanban

---

## Windows-Specific Quirks

OpenClaw runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
OpenClaw的 own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The OpenClaw-installed venv at
...
every OpenClaw tool and most Windows APIs. Prefer forward slashes in code
...
If OpenClaw reports a smaller context window than your local model supports
(e.g., 128k when llama-server has `-c 262144`):

**Check if `model.context_length` is explicitly set.** OpenClaw uses a
...
|----------------|----------|
| Config options | `hermes config edit` or [Configuration docs](https://openclaw.nousresearch.com/docs/user-guide/configuration) |
| Available tools | `hermes tools list` or [Tools reference](https://openclaw.nousresearch.com/docs/reference/tools-reference) |
| Slash commands | `/help` in session or [Slash commands reference](https://openclaw.nousresearch.com/docs/reference/slash-commands) |
| Skills catalog | `hermes skills browse` or [Skills catalog](https://openclaw.nousresearch.com/docs/reference/skills-catalog) |
| Provider setup | `hermes model` or [Providers guide](https://openclaw.nousresearch.com/docs/integrations/providers) |
| Platform setup | `hermes gateway setup` or [Messaging docs](https://openclaw.nousresearch.com/docs/user-guide/messaging/) |
| MCP servers | `hermes mcp list` or [MCP guide](https://openclaw.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `hermes profile list` or [Profiles docs](https://openclaw.nousresearch.com/docs/user-guide/profiles) |
| Cron jobs | `hermes cron list` or [Cron docs](https://openclaw.nousresearch.com/docs/user-guide/features/cron) |
| Memory | `hermes memory status` or [Memory docs](https://openclaw.nousresearch.com/docs/user-guide/features/memory) |
| Env variables | `hermes config env-path` or [Env vars reference](https://openclaw.nousresearch.com/docs/reference/environment-variables) |
| CLI commands | `hermes --help` or [CLI reference](https://openclaw.nousresearch.com/docs/reference/cli-commands) |
...
For occasional contributors and PR authors. Full developer docs: https://openclaw.nousresearch.com/docs/developer-guide/
...
**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The OpenClaw-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site (`/c/Program Files/Python311/python -m pip install --user pytest pytest-xdist pyyaml`), then run tests directly:
