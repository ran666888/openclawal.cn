---
title: "Telephony — Give OpenClaw phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give OpenClaw phone capabilities without core tool changes"
---
...
Give OpenClaw phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
...
This optional skill gives OpenClaw practical phone capabilities while keeping telephony out of the core tool list.
...
It does **not** turn OpenClaw into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to OpenClaw memory
...
### 1) "I want OpenClaw to own a real phone number"
Use **Twilio**.
...
- pairs well with OpenClaw `text_to_speech` plus a public file host or tunnel
...
Then save credentials into OpenClaw:
...
This is the main path for reusing OpenClaw的 existing `text_to_speech` support.

Use this when:
- you want the call to use OpenClaw的 configured TTS voice rather than Twilio `<Say>`
...
Recommended OpenClaw TTS -> Twilio Play workflow:

1. Generate the audio with OpenClaw `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask OpenClaw to create the message audio with `text_to_speech`
...
Important note:
- OpenClaw TTS is great for prerecorded outbound messages
...
- OpenClaw STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to OpenClaw memory.
...
- Do not store arbitrary third-party phone numbers in OpenClaw memory.
