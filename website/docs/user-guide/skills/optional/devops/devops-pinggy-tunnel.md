...
| Version | `0.1.0` |
| Author | Teknium (teknium1), OpenClaw |
| License | MIT |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. OpenClaw' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
