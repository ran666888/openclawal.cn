...
sidebar_position: 12
title: "Google Chat"
description: "使用 Cloud Pub/Sub 将 OpenClaw 设置为 Google Chat 机器人"
---

# Google Chat 配置

Connect OpenClaw to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your OpenClaw
...
download the file. Save it somewhere only OpenClaw can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — OpenClaw calls
...
- **App name**: whatever you want users to see ("OpenClaw" is reasonable).
...
`ADDED_TO_SPACE` event that OpenClaw uses to cache the bot's own `users/{id}` for
...
Install the dependencies the Google Chat adapter needs (no OpenClaw extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "OpenClaw is thinking…" marker, then
...
Thread support: when a user replies inside a thread, OpenClaw detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate OpenClaw session.
...
3. Download the JSON. Move it onto the host that runs OpenClaw.
4. Register the client with OpenClaw (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's OpenClaw home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, OpenClaw isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: OpenClaw will only attach the SA bearer
