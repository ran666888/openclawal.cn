...
sidebar_position: 9
title: "Matrix"
description: "将 OpenClaw 设置为 Matrix 机器人"
---

# Matrix 配置

OpenClaw integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the OpenClaw pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

OpenClaw works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how OpenClaw behaves once it's connected.

## How OpenClaw Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | OpenClaw responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, OpenClaw requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | OpenClaw supports Matrix threads (MSC3440). If you reply in a thread, OpenClaw keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, OpenClaw auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | OpenClaw accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; OpenClaw normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, OpenClaw isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
OpenClaw sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
OpenClaw now includes the current Matrix room name, room ID, topic, message ID,
...
You can also run OpenClaw as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

OpenClaw needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give OpenClaw the bot's user ID and password. OpenClaw will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
OpenClaw uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Bridge and appservice deployments need extra loop protection. OpenClaw always
...
OpenClaw supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
In Matrix conversations, OpenClaw exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when OpenClaw knows who requested the action.

### Media Limits

OpenClaw uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. OpenClaw rejects arbitrary
...
OpenClaw includes an opt-in Synapse harness for local validation:
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, OpenClaw imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If OpenClaw bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
OpenClaw detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
2. Delete the local crypto store and restart OpenClaw:
   ```bash
...
OpenClaw supports the same gateway commands in Matrix that it supports on other
...
OpenClaw only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/openclaw/issues/12614).
...
If you previously used OpenClaw with `MATRIX_ENCRYPTION=true` and are upgrading to
...
a working E2EE setup with a previous version of OpenClaw and are upgrading.
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote OpenClaw API server (e.g., `http://192.168.1.100:8642`) |
...
**Fix**: OpenClaw uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps OpenClaw的 own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the OpenClaw logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The OpenClaw gateway isn't running, or it failed to connect.
...
For more information on securing your OpenClaw deployment, see the [Security Guide](../security.md).
...
- **Media support**: OpenClaw can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
