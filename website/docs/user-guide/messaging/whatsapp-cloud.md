...
description: "通过 Meta 官方 Business Cloud API 将 OpenClaw 设置为 WhatsApp 机器人"
---

# WhatsApp Business Cloud API 配置

OpenClaw can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The OpenClaw gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a OpenClaw limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. OpenClaw degrades gracefully if absent.
...
The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the OpenClaw gateway has to be reachable from Meta's servers.  Three common ways:
...
3. Start the OpenClaw gateway: `hermes gateway`.
...
## Allowlist (OpenClaw-side)

In addition to Meta's recipient whitelist, OpenClaw has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.hermes/.env`:
...
When the agent invokes any of these flows, OpenClaw uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
OpenClaw acknowledges inbound messages immediately:
...
OpenClaw TTS produces MP3. Two paths:
...
OpenClaw warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in OpenClaw. If you need it, please [open an issue](https://github.com/NousResearch/openclaw/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  OpenClaw的 `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  OpenClaw doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **App Secret not set** — without it, OpenClaw refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in OpenClaw.
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that OpenClaw will accept as authentic.
...
Most users running OpenClaw for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
