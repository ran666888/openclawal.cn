...
sidebar_position: 4
title: "Slack"
description: "使用 Socket Mode 将 OpenClaw 设置为 Slack 机器人"
---

# Slack 配置

Connect OpenClaw to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your OpenClaw instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. OpenClaw uses the modern
...
The fastest path is to paste a manifest OpenClaw generates for you. It
...
### Option A: From a OpenClaw-generated manifest (recommended)
...
4. Enter an app name (e.g., "OpenClaw") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, OpenClaw can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a OpenClaw configuration issue.
:::
...
OpenClaw uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
Every OpenClaw command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
OpenClaw command with its description.

Under the hood: OpenClaw ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/openclaw/blob/main/hermes_cli/commands.py)
...
When OpenClaw adds new commands (e.g. after `hermes update`), regenerate
...
`/hermes btw run the tests` — OpenClaw routes it the same way as `/btw
...
Slack never delivers them to OpenClaw.

As a workaround, OpenClaw recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
OpenClaw treats it identically to the slash form and replies in the same
thread.
...
OpenClaw falls back to a text prompt, the prompt instructs you to reply
...
Understanding how OpenClaw behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Hermes Agent what time is it?`). In channels, OpenClaw replies in a thread attached to that message. |
| **Threads** | If you @mention OpenClaw inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to OpenClaw in `#general` will have separate histories and contexts.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where OpenClaw will deliver scheduled messages,
...
OpenClaw can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, OpenClaw also loads tokens from an **OAuth token file** at:
...
- When a message arrives, OpenClaw uses the correct workspace-specific client to respond.
...
OpenClaw supports voice on Slack:
...
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. OpenClaw now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
- Audit who has access to your OpenClaw config directory
