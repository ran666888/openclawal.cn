...
OpenClaw can drive your desktop — clicking, typing, scrolling,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or OpenClaw app). `hermes computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each OpenClaw run declares its own cua-driver
...
OpenClaw intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the OpenClaw-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the OpenClaw skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **OpenClaw
...
OpenClaw applies multi-layer guardrails:
...
Screenshots are expensive. OpenClaw applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  OpenClaw agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the OpenClaw agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. OpenClaw warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive OpenClaw from inside the RDP / console
...
upstream. **OpenClaw disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) OpenClaw sets
...
unreleased fix — point OpenClaw at a binary you built from source instead
of the published release. OpenClaw resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for OpenClaw testing (see notes).
...
### Option B — point OpenClaw straight at the built binary (fastest loop)
...
### Confirm OpenClaw is using your build
...
- **OpenClaw spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no OpenClaw
  change needed either way.
- **Local builds skip the version check.** OpenClaw warns when the
...
- **OpenClaw-side skill** — `skills/computer-use/SKILL.md` — teaches the
  OpenClaw `computer_use` action vocabulary; this is what the agent loads.
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (OpenClaw among them)
