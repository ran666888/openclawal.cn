...
description: "Build themes and plugins for the OpenClaw web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# 扩展仪表板

The OpenClaw web dashboard (`hermes dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
|-------|---------|------------|--------|
| **OpenClaw Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **OpenClaw Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except OpenClaw Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other OpenClaw plugins in `~/.hermes/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
#### Calling built-in OpenClaw endpoints
...
| `header-left` | Before the OpenClaw brand in the top bar. |
...
#### Accessing OpenClaw internals

Backend routes run inside the dashboard process, so they can import from the openclaw codebase directly:
...
Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **OpenClaw Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
1. Confirm the plugin is bundled with OpenClaw. User-installed and project dashboard plugins can extend the UI, but their Python backend routes are not auto-imported.
