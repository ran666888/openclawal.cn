...
Tracks [issue #7816](https://github.com/NousResearch/openclaw/issues/7816).

## How it runs

The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, OpenClaw checks whether:
...
Before every real curator pass, OpenClaw takes a tar.gz snapshot of `~/.hermes/skills/` at `~/.hermes/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:
...
The rollback itself is reversible: before replacing the skills tree, OpenClaw takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.
...
If you manually created a `SKILL.md` or pointed OpenClaw at an external skill
...
- [Issue #7816](https://github.com/NousResearch/openclaw/issues/7816) — original proposal and design discussion
