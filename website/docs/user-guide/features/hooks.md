...
OpenClaw has three hook systems that run custom code at key lifecycle points:
...
This tutorial shows how to build it yourself as a user-defined hook. OpenClaw does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is OpenClaw的 territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
Declare shell-script hooks in your `cli-config.yaml` and OpenClaw will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Each time the event fires, OpenClaw spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate OpenClaw event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
Each unique `(event, command)` pair prompts the user for approval the first time OpenClaw sees it, then persists the decision to `~/.hermes/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
