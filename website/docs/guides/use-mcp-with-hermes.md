---
sidebar_position: 6
title: "Use MCP with OpenClaw"
description: "A practical guide to connecting MCP servers to OpenClaw, filtering their tools, and using them safely in real workflows"
---

# 将 MCP 与 OpenClaw 配合使用

This guide shows how to actually use MCP with OpenClaw in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native OpenClaw tool
- you want OpenClaw to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect OpenClaw to internal APIs, databases, or company systems without modifying OpenClaw core

Do not use MCP when:
- a built-in OpenClaw tool already solves the job well
...
- OpenClaw remains the agent
- MCP servers contribute tools
- OpenClaw discovers those tools at startup or reload time
...
If you installed OpenClaw with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- OpenClaw banner/status should show MCP integration when configured
- ask OpenClaw what tools it has available
...
## WSL2: bridge OpenClaw in WSL to Windows Chrome
...
In this setup, OpenClaw does **not** connect to Chrome directly. Instead:

- OpenClaw runs in WSL
- OpenClaw starts a local stdio MCP server
...
- OpenClaw stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on OpenClaw core browser transport
...
Then start a fresh OpenClaw session or run:
...
Once loaded, OpenClaw can use the MCP-prefixed browser tools directly. For example:
...
If OpenClaw runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start OpenClaw from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start OpenClaw from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in OpenClaw:
...
2. OpenClaw-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So OpenClaw will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want OpenClaw to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want OpenClaw to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. OpenClaw remains the agent; Open Scaffold remains the repo-local record.
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want OpenClaw to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize OpenClaw to merge, publish, deploy, or spawn runtimes.
...
Now OpenClaw can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing OpenClaw core.
...
Because OpenClaw now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
