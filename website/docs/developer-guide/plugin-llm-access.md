...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept OpenClaw already has applies. The plugin never sees a
...
openclaw itself). For the async surface (`acomplete()` /
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through OpenClaw' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing OpenClaw subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/openclaw/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/openclaw/blob/main/tests/agent/test_plugin_llm.py)
