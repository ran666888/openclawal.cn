...
This guide covers adding a new messaging platform to the OpenClaw gateway. A platform adapter connects OpenClaw to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core OpenClaw code. Your plugin is a directory with two files:
...
:::note
This checklist is for adding a platform directly to the OpenClaw core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
